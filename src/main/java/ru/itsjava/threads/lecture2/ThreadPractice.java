...
//        Runnable runnable = () -> {
//            for (int i = 0; i < 5; i++) {
//                System.out.println("Привет посла сна в 3 секунды");
//                try {
//                    Thread.sleep(3000);
//                } catch (InterruptedException e) {
//                    e.printStackTrace();
...
//        thread1.start();
        ...
        Runnable lymbdaRunnable = () -> System.out.println("Привет я Runnable");
        new Thread(lymbdaRunnable).start();

        Runnable lymbdaSleepRunnable = () -> {
            for (int i = 0; i < 5; i++) {
        ...
        Thread thread = new Thread(lymbdaSleepRunnable);
